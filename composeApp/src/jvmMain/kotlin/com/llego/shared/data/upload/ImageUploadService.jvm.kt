...
class JvmImageUploadService : ImageUploadService {
    ...
    private suspend fun uploadImage(
        ...
        return try {
            ...
            val response = client.submitFormWithBinaryData(
                ...
                formData = formData {
                    append(fileField, file.readBytes(), Headers.build {
    ...
    private suspend fun detectFromShowcase(
        ...
        return try {
            val file = File(filePath)
            if (!file.exists()) {
                return ShowcaseDetectionResult.Error("El archivo no existe: $filePath")
            }
    ...
    override suspend fun uploadAdCreative(imageBytes: ByteArray, token: String?): ImageUploadResult {
        return try {
            ...
            ImageUploadResult.Error("Error al subir creativo: ${e.message}")
        }
    }
}
